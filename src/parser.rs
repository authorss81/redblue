...
impl Parser {
            ...
            TokenKind::Import => {
                ...
                loop {
                    if let Some(Token { kind: TokenKind::Identifier(name), .. }) = self.current() {
                        ...
                        let alias = if let Some(Token { kind: TokenKind::To, .. }) = self.current() {
                            self.advance();
                            if let Some(Token { kind: TokenKind::Identifier(alias), .. }) = self.current() {
                                let a = alias.clone();
                                self.advance();
                                Some(a)
                            } else {
    ...
    fn parse_test(&mut self) -> Result<Option<Statement>> {
        ...
        let name = match self.current() {
            Some(Token { kind: TokenKind::Text(s), .. }) => {
                let name = s.clone();
                self.advance();
                name
            },
